...
public class VentanaBuses extends javax.swing.JFrame {
    ...
    private void buscarBusesPorRuta() {
        ...
        if (idRutaTexto.isEmpty()) {
            ...
            return; // Salir del método si el texto está vacío
        }

        int idRuta;
        try {
            comboBoxBuses.removeAllItems();
            idRuta = Integer.parseInt(idRutaTexto); // Convertir el texto a un número entero
        ...
        Ruta ruta = null;
        try {
            ruta = empresa.obtenerRuta(idRuta); // Consultar la ruta en la empresa
        } catch (RutaNoEncontradaException e) {
            // Mostrar un mensaje de error si la ruta no se encuentra
        ...
        if (ruta != null) {
            ...
            // Llenar el ComboBox con los buses obtenidos
            for (Bus bus : buses) {
    ...
    private void confirmarNuevoBusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_confirmarNuevoBusActionPerformed
        ...
        try {
            ...
            // Convertir el campo de costo de mantención a tipo double
            double costoMantencion = Double.parseDouble(costoMantencionText);

            // Convertir el campo de ruta a tipo int
            int idRuta = Integer.parseInt(rutaNuevo);
            ...
            nuevoBus.setPasajerosActuales(0); // Inicializar el número de pasajeros actuales a 0

            // Intentar agregar el bus a la base de datos
            boolean exitoBD = agregarBusBD(nuevoBus, idRuta);

            if (exitoBD) {
                // Si se agregó correctamente a la base de datos, agregar el bus a la lista de la empresa
            ...
            // Limpiar los campos de texto después de agregar el bus
            textoNuevoBus.setText("");
            textoRutaNuevoBus.setText("");
            textoCostoMantencionBus.setText("");

        } catch (CampoVacioException e) {
            // Mostrar mensaje de error si hay un campo vacío
        ...
        // Cerrar el diálogo de agregar bus
        dialogoAgregarBus.dispose();
    }//GEN-LAST:event_confirmarNuevoBusActionPerformed
    private boolean agregarBusBD(Bus bus, int idRuta) {
        // Obtener la conexión a la base de datos desde la empresa
        Connection cone = empresa.getConexionBd();
        // Consulta SQL para insertar un nuevo bus en la tabla Buses
        ...
        try (PreparedStatement pstmt = cone.prepareStatement(query)) {
            ...
            pstmt.setString(1, bus.getPatente().toUpperCase()); // Establecer la patente del bus
            pstmt.setInt(2, bus.getCapacidad()); // Establecer la capacidad del bus
            pstmt.setInt(3, bus.getPasajerosActuales()); // Establecer el número actual de pasajeros (inicialmente puede ser 0)
            pstmt.setDouble(4, bus.getCostoMantencion()); // Establecer el costo de mantención del bus
            pstmt.setInt(5, idRuta); // Establecer el ID de la ruta del bus
    ...
    private boolean rutaExists(int idRuta, Connection conn) {
        // Consulta SQL para verificar si existe una ruta con el ID proporcionado
        ...
        try {
            ...
            // Establecer el parámetro de la consulta (ID de la ruta)
            pstmt.setInt(1, idRuta);

            // Ejecutar la consulta y obtener el resultado
            ResultSet rs = pstmt.executeQuery();

            // Retornar true si hay al menos una coincidencia (el conteo es mayor a 0)
            return rs.next() && rs.getInt(1) > 0;
        } catch (SQLException e) {
            // Manejar la excepción SQL y retornar false en caso de error
            return false;
        }
    }

    private boolean patenteExists(String patente, Connection conn) {
        // Consulta SQL para verificar si existe un bus con la patente proporcionada
        ...
        try {
            ...
            // Establecer el parámetro de la consulta (patente del bus)
    ...
    private void confirmarEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_confirmarEliminarActionPerformed
        ...
        // Obtener la patente del bus del campo de texto y convertirla a mayúsculas
        String patenteBus = textoFieldEliminarBus.getText().toUpperCase();

        // Verificar si el campo de la patente está vacío
        if (patenteBus.isEmpty()) {
            // Mostrar un mensaje de error si la patente está vacía
            ...
            if (confirmacion == jOptionPane.YES_OPTION) {
                try {
                    // Intentar obtener el bus usando la patente proporcionada
                    ...
                    if (bus != null) {
                        ...
                        if (empresa.eliminarBusEmpresa(bus)) {
                            // Mostrar un mensaje de éxito si el bus fue eliminado
                            jOptionPane2.showMessageDialog(this, "Bus eliminado correctamente.", "Éxito", jOptionPane1.INFORMATION_MESSAGE);
                        }
                    } else {
                        // Mostrar un mensaje de error si no se encuentra el bus
                        jOptionPane2.showMessageDialog(this, "No se encontró un bus con esa patente.", "Error", jOptionPane1.ERROR_MESSAGE);
                    }
                    ...
                    // Mostrar un mensaje de error si ocurre una excepción durante la eliminación
                    jOptionPane1.showMessageDialog(this, "Ocurrió un error al intentar eliminar el bus.", "Error", jOptionPane1.ERROR_MESSAGE);
                }
            } else {
                // Mostrar un mensaje indicando que la operación fue cancelada
    ...
    private void confirmarMBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_confirmarMBActionPerformed
       try 
       {
                ...
                boolean exitoBD = modificarBusBD(patenteBus.toUpperCase(), capacidadNueva, nuevoCosto);

                if (exitoBD) {
                    // Actualizar el bus en la empresa
                    empresa.modificarBus(patenteBus.toUpperCase(), capacidadNueva, nuevoCosto);
    ...
    private boolean modificarBusBD(String patente, int nuevaCapacidad, double nuevoCosto) 
    {
             ...
             PreparedStatement pstmt = conn.prepareStatement(query)) 
        {
            ...
            pstmt.setString(3, patente.toUpperCase());
